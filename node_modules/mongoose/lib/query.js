...
function Query(conditions, options, model, collection) {
  ...
  // this is needed because map reduce returns a models that can be queried, but
  // all of the queries on said models should be lean
 ...
 *     var TestModel = db.models('Test', TestSchema);
 ...
 * ####Example:
 *
 *     var countQuery = models.where({ 'color': 'black' }).count();
 *
 ...
 * ####Example:
 *
 *     const countQuery = models.where({ 'color': 'black' }).countDocuments();
 *
...
/*!
 * If the models is a discriminator type and not root, then add the key & value to the criteria.
 */
 ...
 * - `runValidators`: if true, runs [update validators](/docs/validation.html#update-validators) on this command. Update validators validate the update operation against the models's schema.
 * - `setDefaultsOnInsert`: if this and `upsert` are true, mongoose will apply the [defaults](http://mongoosejs.com/docs/defaults.html) specified in the models's schema if a new document is created. This option only works on MongoDB >= 2.4 because it relies on [MongoDB's `$setOnInsert` operator](https://docs.mongodb.org/v2.4/reference/operator/update/setOnInsert/).
 ...
 * @param {Model} [models] The models you wish to use for population. If not specified, populate will look up the models by the name in the Schema's `ref` field.
...
/**
 * Casts this query to the schema of `models`
 *
 * ####Note
 *
