 ...
 * - 'cloneSchemas': false by default. Set to `true` to `clone()` all schemas before compiling into a models.
...
/**
 * Defines a models or retrieves it.
 *
 ...
 *
 *     // define an Actor models with this mongoose instance
 *     mongoose.models('Actor', new Schema({ name: String }));
 *
 *     // create a new connection
 *     var conn = mongoose.createConnection(..);
 *
 *     // retrieve the Actor models
 *     var Actor = conn.models('Actor');
 *
 * _When no `collection` argument is passed, Mongoose uses the models name. If you don't like this behavior, either pass a collection name, use `mongoose.pluralize()`, or set your schemas collection name option._
 *
 * ####Example:
 *
 ...
 *     var M = mongoose.models('Actor', schema, collectionName)
 *
 * @param {String|Function} name models name or class extending Model
 * @param {Schema} [schema]
 * @param {String} [collection] name (optional, inferred from models name)
...
Mongoose.prototype.model = function(name, schema, collection, skipInit) {
  ...
  if (schema && !schema.instanceOfSchema) {
    throw new Error('The 2nd parameter to `mongoose.models()` should be a ' +
      'schema or a POJO');
  }
  ...
  // handle internal options from connection.models()
  let options;
  ...
  // connection.models() may be passing a different schema for
  // an existing models name. in this case don't read from cache.
  if (this.models[name] && options.cache !== false) {
    ...
    if (collection) {
      // subclass current models with alternate collection
      model = this.models[name];
      schema = model.prototype.schema;
      sub = model.__subclass(this.connection, schema, collection);
      // do not cache the sub models
      return sub;
    }
...
/**
 * Returns an array of models names created on this instance of Mongoose.
 *
 * ####Note:
 *
 * _Does not include names of models created using `connection.models()`._
 *
 * @api public
 * @return {Array}
 */
 ...
 *
 * This is the connection used by default for every models created using [mongoose.models](#index_Mongoose-models).
 *
 * @memberOf Mongoose
 * @instance
 * @property connection
